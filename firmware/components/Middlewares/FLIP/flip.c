...
static void transfer_velocities(int to_grid, float flip_ratio,
    ...
    if (to_grid) {
        memcpy(prev_u, u, sizeof(float) * f_num_x * f_num_y);
        memcpy(prev_v, v, sizeof(float) * f_num_x * f_num_y);
        memset(du, 0, sizeof(float) * f_num_x * f_num_y);
        memset(dv, 0, sizeof(float) * f_num_x * f_num_y);
        memset(u, 0, sizeof(float) * f_num_x * f_num_y);
        memset(v, 0, sizeof(float) * f_num_x * f_num_y);
...
static void solve_incompressibility(
    ...
    const int n = f_num_y;
    memset(p, 0, sizeof(float) * f_num_x * f_num_y);
    memcpy(prev_u, u, sizeof(float) * f_num_x * f_num_y);
    memcpy(prev_v, v, sizeof(float) * f_num_x * f_num_y);
...
FlipFluid* flip_create(float sim_w, float sim_h, int visible_res,
    ...
    int max_particles = MAX(num_x * num_y + 1000, 3000);
...
void flip_step(FlipFluid* f, float dt, float gx, float gy) {
    ...
    transfer_velocities(1, /*flip_ratio=*/0.9f, f->num_particles,
    ...
    solve_incompressibility(
        /*num_iters=*/20, dt, /*over_relaxation=*/1.9f, /*compensate_drift=*/1,
    ...
    transfer_velocities(0, /*flip_ratio=*/0.9f, f->num_particles,
                        f->particle_pos, f->particle_vel, f->u, f->v, f->du,
                        f->dv, f->prev_u, f->prev_v, f->cell_type, f->s,
                        f->f_num_x, f->f_num_y, f->h, f->f_inv_spacing,
                        f->AIR_CELL, f->FLUID_CELL, f->SOLID_CELL);
